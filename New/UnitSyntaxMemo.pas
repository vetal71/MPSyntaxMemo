...
begin
    ...
    fTokenStyles[tokChar].tsStyle               := [fsItalic, fsUnderline];
    fTokenStyles[tokCharEnd].tsForeground       := clBlue;
    fTokenStyles[tokCharEnd].tsStyle            := [fsItalic, fsUnderline];
...
function TMPCustomSyntaxMemo.PixOffsetToWndOffsetEx(const CharPix, ScreenRow: LongInt {Integer}): TPoint;
begin
    with TextRowRect[ ScreenRow ] do begin
//        Inc(TextRowRect[ ScreenRow ].Left, CharPix - OffsetXPix);
        TextRowRect[ ScreenRow ].Left := 100;
        Result := TextRowRect[ ScreenRow ].TopLeft;
    end;
end;
